...
def rain_rate(time, counts_or_tips, tip_bucket_size=None,
    ...
    if counts_or_tips.ndim == 1:
        if (drop_counter_size is None and drop_size is None) & (
                np.max(counts_or_tips) == 1 or tip_bucket_size is not None):
            """TIPPING BUCKET"""

            # If the data follows a tipping bucket structure but tip_bucket_size was not specified, then warn user and
            # set to standard 0.2 mm
            if tip_bucket_size is None:
                warnings.warn('\n[gu.rainrate]: the input data has the same structure as a TIPPING BUCKET but tip_'
                              'bucket_size was not specified. We will assume a standard bucket size of 0.2 mm!',
        ...
        if (tip_bucket_size is None and drop_size is None) & (np.sum(counts_or_tips > 0) > 10):
            ...
            if drop_counter_size is None:
                warnings.warn('\n[gu.rainrate]: the input data has the same structure as a DROP COUNTER but drop_'
                              'counter_size was not specified. We will assume a standard bucket size of 0.0033 mm!',
                              SyntaxWarning, stacklevel=2)
        ...
        if mask is True:
            ...
            Time_Disd = 0.5 * np.diff(time) + time[:-1];
            Time_Disd = np.append(Time_Disd, Time_Disd[0] - dt / 3600);
            Time_Disd = np.append(Time_Disd, Time_Disd[-1] + dt / 3600)
...
def Drops2RR(drop_time, drop_count, drop_size):
    ...
    # Initialise Equations and Constants
...
def Cloud_Identifer(Time, Sg, Sd):
    ...
    if Time[-1] - Time[0] < 0.25:
        warnings.warn(
            '[Cloud_Identifer]: The identification of cloud type requires 15 minutes or more data. Only %.2f minutes was found!' % (
                    (Time[-1] - Time[0]) * 60), RuntimeWarning, stacklevel=2)
        return None, None
    ...
    # Determine if Time is longer than 15 minutes
        ...
        with warnings.catch_warnings():
            ...
            Cloud_Time = np.array([Time[0] + 0.125 + Time_Step for Time_Step in
                                   np.linspace(0, Cloud_Num * 0.25, Cloud_Num, endpoint=False)], dtype=float)
            ...
            if Datetime_Found == 'datetime': Cloud_Time = np.array(
                [roundTime(Date + timedelta(hours=hour_frac), 1) for hour_frac in Cloud_Time], dtype=object)
...
def cloud_identifer_ascent(height, rh_ice, method='Zhang', verbose=False):
    ...
    for Cloud_Below, Cloud_Above in zip(np.unique(Clouds_ID)[1:-1], np.unique(Clouds_ID)[2:]):
        ...
        if ((height[Clouds_ID == Cloud_Above][0] - height[Clouds_ID == Cloud_Below][-1]) < 0.3) or (
                np.nanmin(rh_ice[Air_Between]) > np.nanmax(interRH[Air_Between])):
    ...
    if verbose is True:
        ...
        for Cloud in np.unique(Clouds_ID)[1:]:
            print("Cloud %s. Cloud Base = %.2fkm, Cloud Top = %.2fkm, Layer Type: %s" % (
                Cloud, height[Clouds_ID == Cloud][0], height[Clouds_ID == Cloud][-1],
                Cloud_Types[LayerType[Clouds_ID == Cloud][0]]))
